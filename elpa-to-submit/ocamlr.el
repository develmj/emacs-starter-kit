          ...
          (mapcar (lambda (name)
                    (let ((desc (eval (intern name)))
                      ...
                      (if (or (and (functionp 'find-face) (find-face face))
                        ...
                        (if use-fonts
                            (progn
                              (if (nth 4 desc) (ocamlr-make-face-italic face)
                                (ocamlr-make-face-unitalic face))
                              (if (nth 5 desc) (ocamlr-make-face-bold face)
                                (ocamlr-make-face-unbold face)))
                          (if (nth 2 desc) (ocamlr-make-face-italic face)
                            (ocamlr-make-face-unitalic face))
                          (if (nth 3 desc) (ocamlr-make-face-bold face)
                            (ocamlr-make-face-unbold face))))))
                  '("ocamlr-font-lock-governing"
                    "ocamlr-font-lock-operator"
                    "ocamlr-font-lock-interactive-output"
                    "ocamlr-font-lock-interactive-error"))))

      ;; FIXME
      ;; les commentaires sont correctement en rouge tout va bien
      ;; les indications au préprocesseur dovent être en rose pale
      ;; dans une déclaration de type les noms de variables en jaune-orange pâle
      ;; les types standards et les nom de classe en vert
      ;; les nom de fonctions dans les déclarations en bleu
      ;; les mot-clés du langages en violet
      ;; Les importations pourrait se faire comment en java mot-clé en violet 
      ;; paquetage importé en vert pâle
      ;; dans les commentaires en javadoc en vert pâle le nom des tags,
      ;; en jaune type les params
      ;; Dans ce même vert pâle les valeurs standards de langage True False etc
      ;; FIXME: dans un futur lointain je mettrais en vert tout les types, et
      ;; jaune clair que l'utilisateur veut bien créer.


      ;; FIXME: definir les faces de emacs21 pour xemacs 
      ;; càd builtin-face défini à partir de preprocessor-face
      ;;     constant-face défini à partir de reference-face
      
      ;; FIXME comme cc, on définit les faces par group c'est plus puissant
      ;; Dans un futur lointain, séparer en trois niveau

;; val is no longer a keyword
      (defvar ocamlr-font-lock-keywords
         (list
          '("^#\\(\\w+\\) \"?\\(\\w+\\)\"?;" (1 'font-lock-builtin-face) (2 'font-lock-variable-name-face))
          '("\\<\\(external\\|open\\|include\\)\\>[ \t\n]*\\([_A-Za-z\277-\377]\\(\\w\\|[._]\\)*\\);" (1 'font-lock-builtin-face) (2 'font-lock-constant-face) nil)
          '("\\<\\(and\\|as\\|assert\\|asr\\|closed\\|constraint\\|do\\|done\\|downto\\|else\\|for\\|fun\\|functor\\|if\\|in\\|include\\|land\\|lazy\\|lor\\|lsl\\|lsr\\|lxor\\|match\\|mod\\|mutable\\|new\\|of\\|or\\|parse\\|parser\\|private\\|ref\\|then\\|to\\|try\\|virtual\\|when\\|where\\|while\\|with\\)\\>" 0 'font-lock-keyword-face nil)
...
;;          (progn (message "C UNE DECLARATION DE PHRASE") (current-column)))
         ...
         ((save-excursion (looking-at "in\\>"))
          (progn (message "LOOKING AT IN") (re-search-backward "\\<let\\s-*.+\\s-*=.*")
                 (current-column)))
         ((save-excursion (looking-at "then\\>"))
...
;;            (forward-line -1) ;; Remplacer cette fonction par une qui 
            ...
            (cond 
             ...
             ;; Process let .+ in
             ((looking-at "let\\>.+\\<in\\s-*$")
              (progn (message "LOOKING AT LET BODY") 
                     (current-column)))
             ;; Process a with pattern match
             ((looking-at "with\\>\\s-*$")
